...
import { codeProvince } from "../../utils/normalize";
import { OrderSchemaExpected } from "../../utils/schemas/schema";
import {
  IFallbackData,
  IShopifyAddress,
...
export function convertToOrderSchemaExpected(input: any): {
  ...
  const orderSchemaExpected: OrderSchemaExpected = {
    ...
    billing_address: {
      country: checkCritical(billingAddress.countryCode),
      ...
      state_code:
        codeProvince(billingAddress.provinceCode, "PE-") || "Sin datos"
    },
    shipping_address: {
      country: checkCritical(shippingAddress.countryCode),
      ...
      state_code:
        codeProvince(shippingAddress.provinceCode, "PE-") || "Sin datos"
    },
    customer: {
...
const obtainDistrictData = (
  billingAddress: any,
  shippingAddress: any,
  tags: string[]
) => {
  const isValidateReleasit = tags.includes("releasit_cod_form");
  return {
    bilingAddres: {
      state: isValidateReleasit ? billingAddress?.state : billingAddress?.state,
      city: isValidateReleasit ? billingAddress?.city : billingAddress?.state,
      district: isValidateReleasit ? billingAddress?.city : billingAddress?.city
    },
    shippingAddress: {
      state: isValidateReleasit
        ? shippingAddress?.state
        : shippingAddress?.state,
      city: isValidateReleasit ? shippingAddress?.city : shippingAddress.state,
      district: isValidateReleasit
...
const buildProcessOrderBody = (
  ...
  const addressData = obtainDistrictData(
  ...
  const result = {
    ...
    origin_address: {
      zip: "0000",
      country: "PE",
      city: addressData?.bilingAddres?.city || null,
      ...
      company: null,
      state: addressData?.bilingAddres?.state || null,
      district: addressData?.bilingAddres?.district || null,
    ...
    shipping_address: {
      zip: "0000",
      country: "PE",
      city: addressData?.shippingAddress?.city || null,
      ...
      company: null,
      state: addressData?.shippingAddress?.state || null,
      district: addressData?.shippingAddress?.district || null,
